...
def download_invoice():
    ...
    add_product = driver.find_element(By.XPATH, value="//div[@class = 'productinfo text-center']/a")
    ...
    # Click Proceed To Checkout and click registr/login
    checkout_btn = driver.find_element(By.XPATH, value="//a[@class = 'btn btn-default check_out']")
    checkout_btn.click()
    register_login_btn = driver.find_element(By.XPATH, value="//div[@class = 'modal-body']//a")
    register_login_btn.click()
    print('Clicked checkout, reg/log buttons, Done')

    # Filling all details in Signup and create account

    driver.find_element(by=By.XPATH, value="//input[@data-qa = 'signup-name']").send_keys(config.name)
    driver.find_element(by=By.XPATH, value="//input[@data-qa = 'signup-email']").send_keys(config.email)
    driver.find_element(by=By.XPATH, value="//button[@data-qa= 'signup-button']").submit()
    ...
    select3.select_by_value(config.country)
    ...
    # Verification ' Logged in as username' at top
    ...
    success_alert = wait.until(EC.visibility_of_element_located((By.XPATH, "//div[@class = 'col-sm-9 col-sm-offset-1']/p")))
    value4 = success_alert.text
    assert value4 == 'Congratulations! Your order has been confirmed!'
    print('Order payed, Done')

    # Click download invoice and continue
    driver.find_element(By.XPATH, value="//a[@class = 'btn btn-default check_out']").click()
    driver.find_element(By.XPATH, value="//a[@class = 'btn btn-primary']").click()
    print("Invoice downloaded, Done")

    # Click 'Delete Account' button
    driver.find_element(By.XPATH, value="//ul[@class='nav navbar-nav']/li[5]/a").click()

    # verification that account deleted
    v_del = wait.until(EC.visibility_of_element_located((By.XPATH, "//h2[@data-qa='account-deleted']/b")))
    value4 = v_del.text
    assert value4 == "ACCOUNT DELETED!"
